...
class Vision:
    ...
    def __init__(self, pitch, colour, our_side, frame_shape, frame_center, calibration):
        """
        ...
        if our_side == 'left':
            self.us = [
                RobotTracker(
                    colour=colour, crop=zones[0], offset=zones[0][0], pitch=pitch,
                    name='Our Defender', calibration=calibration),
                RobotTracker(
                    colour=colour, crop=zones[2], offset=zones[2][0], pitch=pitch,
                    name='Our Attacker', calibration=calibration)
            ]

            self.opponents = [
                RobotTracker(
                    colour=opponent_color, crop=zones[3], offset=zones[3][0], pitch=pitch,
                    name='Their Defender', calibration=calibration),
                RobotTracker(
                    colour=opponent_color, crop=zones[1], offset=zones[1][0], pitch=pitch,
            ...
            self.us = [
                RobotTracker(
                    colour=colour, crop=zones[3], offset=zones[3][0], pitch=pitch,
                    name='Our Defender', calibration=calibration),
                RobotTracker(
                    colour=colour, crop=zones[1], offset=zones[1][0], pitch=pitch,
                    name='Our Attacker', calibration=calibration)
            ]

            self.opponents = [
                RobotTracker(
                    colour=opponent_color, crop=zones[0], offset=zones[0][0], pitch=pitch,
                    name='Their Defender', calibration=calibration),   # defender
                RobotTracker(
                    colour=opponent_color, crop=zones[2], offset=zones[2][0], pitch=pitch,
                    name='Their Attacker', calibration=calibration)
            ]
...
class GUI(object):
    ...
    def draw(self, frame, model_positions, regular_positions, fps, a_state,
             d_state, a_action, d_action, grabbers, our_color, our_side,
        ...
        key_colour_pairs = zip(
            ...
            [our_color, their_colour]*2)

        self.draw_ball(frame, regular_positions['ball'])

        for key, colour in key_colour_pairs:
            self.draw_robot(frame, regular_positions[key], colour)
        ...
        self.draw_states(frame_with_blank, a_state, d_state, (frame_width, frame_height))
        ...
        # Draw center of uncropped frame (test code)
    ...
    def data_text(self, frame, frame_offset, our_side, text, x, y, angle,
                  velocity, a_action, d_action):
        draw_x = y_offset = None
    ...
    def draw_states(self, frame, a_state, d_state, frame_offset):
        frame_width, frame_height = frame_offset
        x_main = lambda zz: (frame_width/4)*zz
        x_offset = 20
        ...
        self.draw_text(frame, a_state[0], x_main(1) - x_offset, y_offset + 15, size=0.6)
        self.draw_text(frame, a_state[1], x_main(1) - x_offset, y_offset + 30, size=0.6)

        self.draw_text(frame, "Defender State:", x_main(2) + x_offset, y_offset, size=0.6)
        self.draw_text(frame, d_state[0], x_main(2) + x_offset, y_offset + 15, size=0.6)
        self.draw_text(frame, d_state[1], x_main(2)+x_offset, y_offset + 30, size=0.6)
